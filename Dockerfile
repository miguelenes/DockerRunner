# base
FROM ubuntu:22.04
MAINTAINER "Miguel Enes<
# set the github runner version
ARG RUNNER_VERSION="2.315.0"

# update the base packages and add a non-sudo user

RUN apt-get update -y && apt-get upgrade -y && DEBIAN_FRONTEND=noninteractive apt-get install curl apt-transport-https ca-certificates \
    software-properties-common -y && useradd -m docker
